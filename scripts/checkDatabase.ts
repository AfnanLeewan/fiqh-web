...
async function checkDatabase() {
  try {
    ...
    const contentNodesExists = collections?.some(
      (col: { name: string }) => col.name === "content_nodes",
    );
    console.log(
    ...
    if (contentNodesExists) {
      ...
      console.log("\n📋 Sample documents:");
      const sampleCategories = await ContentNode.find({
        type: "category",
      }).limit(3);
      ...
      console.log(
        "💡 The collection will be created when you first save data.",
      );
      console.log("💡 Try running: npm run seed-mockup");
    }
