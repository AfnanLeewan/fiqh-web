...
(function () {
  ...
  const getProp = (element, name, fallback) =>
    toNumber(get$9(element, name), fallback);
  ...
  const getDirection = (element) =>
    get$9(element, "direction") === "rtl" ? "rtl" : "ltr";
  ...
  const getCssValue = (element, property) =>
    parseInt(get$9(element, property), 10);
  ...
  const getPercentTableWidth = (table) =>
    getPercentSize(table, get$7, getInner) + "%";
  ...
  const getHeightFrom = (warehouse, table, getHeight, fallback) => {
    ...
    return map$1(rows$1, (row, i) =>
      getDimension(row, i, backups, always, getHeight, fallback),
    );
  };
  ...
  const opEraseRows = (grid, details, _comparator, _genWrappers) => {
    ...
    const maxRowIndex = Math.max(
      extractGridDetails(newGrid).rows.length - 1,
      0,
    );
    return bundle(
      newGrid,
  ...
  const BarManager = (wire) => {
    ...
    const isResizer = (target) =>
      has(target, "ephox-snooker-resizer-bar") ||
      has(target, "ephox-dragster-blocker");
  ...
  const removeDataStyle = (table) => {
    ...
    const removeStyleAttribute = (element) =>
      remove$6(element, "data-mce-style");
  ...
  const getRawValue = (prop) => (editor, elm) => {
    const raw =
      editor.dom.getStyle(elm, prop) || editor.dom.getAttrib(elm, prop);
  ...
  const multiple = (elements) => ({
    tag: "multiple" /* SelectionTypeTag.Multiple */,
    elements,
  });
  const single = (element) => ({
    tag: "single" /* SelectionTypeTag.Single */,
    element,
  });
        ...
        findCell(event.target, isRoot).each((finish) => {
          identify(start, finish, isRoot).each((cellSel) => {
            ...
            if (boxes.length === 1) {
              ...
              const isCellClosestContentEditable = is$2(
  ...
  const getCellsFromFakeSelection = (editor) =>
    filter$2(getCellsFromSelection(editor), (cell) =>
      is$1(cell, ephemera.selectedSelector),
    );
  ...
  const TableCellSelectionHandler = (editor, resizeHandler) => {
    ...
    const onSelection = (cells, start, finish) => {
      ...
      tableOpt.each((table) => {
        ...
        const selectedCells = getCellsFromSelection(editor);
        const otherCellsDom = getOtherCells(
          table,
          { selection: selectedCells },
          generators,
        )
          .map((otherCells) =>
            map(otherCells, (cellArr) => map$1(cellArr, (cell) => cell.dom)),
          )
          .getOrUndefined();
