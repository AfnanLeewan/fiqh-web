...
(function () {
  ...
  const applyRelTargetRules = (rel, isUnsafe) => {
    ...
    const removeTargetRules = (rels) =>
      rels.filter((val) => global$2.inArray(rules, val) === -1);
  ...
  const handleExternalTargets = (href, assumeExternalTargets) => {
    if (
      (assumeExternalTargets ===
        "http" /* AssumeExternalTargets.ALWAYS_HTTP */ ||
        assumeExternalTargets ===
          "https") /* AssumeExternalTargets.ALWAYS_HTTPS */ &&
      !hasProtocol(href)
    ) {
  ...
  const tryProtocolTransform =
    (assumeExternalTargets, defaultLinkProtocol) => (data) => {
      ...
      const suggestProtocol =
        (assumeExternalTargets === 1 /* AssumeExternalTargets.WARN */ &&
          !hasProtocol(url)) ||
  ...
  const setup$1 = (editor) => {
    ...
    const getSelectedLink = () =>
      selectedLink.get().or(getLinkFromSelection(editor));
    const gotoSelectedLink = () =>
      getSelectedLink().each((link) => gotoLink(editor, link));
  ...
  const setupContextToolbars = (editor, openLink) => {
    ...
    const onSetupLink = (buttonApi) => {
      ...
      buttonApi.setEnabled(
        isInAnchor(editor, node) && editor.selection.isEditable(),
      );
      return noop;
    };
    /**
