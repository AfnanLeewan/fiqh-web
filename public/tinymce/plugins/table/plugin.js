...
(function () {
  ...
  const ancestor = (scope, selector, isRoot) =>
    ancestor$1(scope, (e) => is$1(e, selector), isRoot);
  ...
  const children$2 = (scope, predicate) =>
    filter$1(children$3(scope), predicate);
  ...
  const buildClassList = (classList) => {
    ...
    return Optional.some(
      buildListItems([
        { text: "Select...", value: "mce-no-match" },
        ...classList,
      ]),
    );
  };
  ...
  const extractAdvancedStyles = (elm) => {
    ...
    return {
      ...
      backgroundcolor: getRaw$1(element, "background-color")
        .map(rgbToHex)
        .getOr(""),
    };
  };
  ...
  const getSelectionTargets = (editor) => {
    ...
    const setupHandler = (handler) => {
      ...
      return () => {
        changeHandlers.set(
          filter$1(changeHandlers.get(), (h) => h !== handler),
        );
      };
    };
