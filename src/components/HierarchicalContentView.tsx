...
export function HierarchicalContentView({ items, basePath, variant = 'sidebar' }: HierarchicalContentViewProps) {
  const pathname = usePathname();
  const [expanded, setExpanded] = useState<ExpandedState>({});
  const [loadedChildren, setLoadedChildren] = useState<Record<string, ContentNode[]>>({});
  ...
  const toggleExpand = async (node: ContentNode) => {
    ...
    if (isExpanding && node.type !== 'article') {
      ...
      const hasLoadedChildren = loadedChildren[node.id] && loadedChildren[node.id].length > 0;
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case 'article':
        return <ArticleOutlinedIcon fontSize="small" sx={{ color: '#4caf50' }} />;
      default:
        return null;
    }
  };
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      case 'category': return '#ff9800'; // Orange
      case 'chapter': return '#2196f3'; // Blue
      case 'article': return '#4caf50'; // Green
      default: return '#9e9e9e'; // Grey
    }
  };

  const renderNode = (node: ContentNode, depth: number = 0, currentBasePath: string = basePath): React.ReactNode => {
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flex: 1 }}>
            ...
            <Typography 
              ...
              sx={{ 
                ...
                textDecoration: 'none',
                fontSize: '1rem',
                '&:hover': { textDecoration: (!hasChildren && !isComingSoon) ? 'underline' : 'none' },
        ...
        <Collapse in={isExpanded} timeout="auto" unmountOnExit>
          <Box sx={{ mt: 1.5 }}>
            {nodeChildren.length > 0 ? (
              ...
              <Typography variant="body2" color="text.secondary" sx={{ ml: 6, py: 1 }}>
    ...
    return (
      <Paper
        ...
        sx={{
          ...
          '&::-webkit-scrollbar-thumb': { bgcolor: 'rgba(0, 0, 0, 0.2)', borderRadius: '4px', '&:hover': { bgcolor: 'rgba(0, 0, 0, 0.3)' } },
        }}
      >
