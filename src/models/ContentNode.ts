...
ContentNodeSchema.statics.searchContent = function(query: string, limit: number = 8) {
  return this.find({
    $and: [
      { published: true },
      {
...
// Export the model
const ContentNode = mongoose.models.ContentNode || mongoose.model<IContentNode>('ContentNode', ContentNodeSchema);
